...
void *thread_func(void *ptr) {
  queue<thread_args> *q=(queue<thread_args> *)ptr;
  thread_args a;
  bool fin;
  ...
  while(!fin) {
    ingest1(a.input,a.output) ;  
    if(q->empty())    pthread_exit((void*) 0);
    pthread_mutex_lock (&my_mutex);
    cerr << "queue lenght is "<<q->size()<<endl;
    fin = q->empty();
    a = q->front();
    q->pop();
    pthread_mutex_unlock (&my_mutex);    
...
int main(int argc,char **argv) {
  ...
  nthreads = min(nfile,nthreads);
  queue<thread_args> args;
  ...
  for(int i=0;i<nfile;i++) {
    thread_args a;
    a.input = argv[i+2];
    a.output = new char[10];
    sprintf(a.output,"out%d",i);
    args.push(a);
  }

  for(int i=0;i<nthreads;i++) {
    result_code = pthread_create(&threads[i%nthreads], NULL, thread_func, (void *)&args);

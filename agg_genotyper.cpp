...
int fillHeader(bcf_hdr_t *hdr) //fills in the standard stuff for an agg header.
{
    ...
    bcf_hdr_append(hdr, "##INFO=<ID=DPA,Number=1,Type=Integer,Description=\"sum of depth for sites alt genotypes\">");
    bcf_hdr_append(hdr, "##INFO=<ID=DPF,Number=1,Type=Integer,Description=\"sum of basecalls filtered from input prior to site genotyping\">");
    bcf_hdr_append(hdr, "##INFO=<ID=AB,Number=R,Type=Integer,Description=\"sum of allele depths for respective alleles (het samples)\">"); 		
...
#ifdef DEBUG
    cerr << "var_pos = (" <<var_start<<","<<var_stop<<")  var_type="<<var_type<<endl;
#endif
	
    bool dp_open = true;
    assert((int)dp_buf.size()==_nsample);
...
int aggReader::setDepth()
{
	...
	else //indel. this calculates the avg DP across (var_start,var_stop)
	{
	    ...
	    {
		// find weighted sum of dp and minimum GQ
		// There are four overlap cases:
		// variant:       a---------b            (a==b for SNPs)
		// 1.        s--------------------e      s<=a&&e>=b
		// 2.               s---e                s>=a&&e<=b
		// 3.         s-----e                    s<=a&&e>=a
		// 4.                     s-----e        s<=b&&e>=b
		...
		int s = it1->start;
		int e = it1->stop;
		if(it1->depth!=bcf_int32_missing) 	
		{
		    if(s<=a&&e>=b) 
		    {
		...
		if((s<=b&&e>=a) && (min_gq==bcf_int32_missing||it1->gq < min_gq))
		{
		    min_gq = it1->gq;
		}		
		it1++;
	    }
	    //	    cerr << i<<" "<<a<<","<<b <<" "<<min_gq<<endl;
	    _out_dp[i] = (int)(round(num/(float)var_len));
	    _out_gq[i] = min_gq;
	    assert( (_out_dp[i]==bcf_int32_missing||_out_dp[i]>=0) && (_out_gq[i]==bcf_int32_missing||_out_gq[i]>=0) );
	}
    }
...
int aggReader::next()
{
	...
	setDepth();
#ifdef DEBUG	
	cerr << var_start+1 <<"-"<<var_stop+1 << endl;
#endif
	int offset=0;
...
void aggReader::annotate_line()
{
	...
	if(vr->gt[i*2+1]!=bcf_gt_missing && vr->gt[i*2]!=bcf_gt_missing)
	{
	    ...
	    if(bcf_gt_allele(vr->gt[i*2]) != bcf_gt_allele(vr->gt[i*2+1]))//ab
	    {
		sum_ab[0] += vr->ad[i*2];		
		sum_ab[1] += vr->ad[i*2+1];
	    }
	}
